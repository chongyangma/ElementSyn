...
void CArcBall::Init()
{
    ...
    ball_data.sets[CameraAxes] = mId[X];
    ball_data.setSizes[CameraAxes] = 3;
    ball_data.sets[BodyAxes] = ball_data.mDown[X];
    ball_data.setSizes[BodyAxes] = 3;
    ball_data.sets[OtherAxes] = otherAxis[X];
    ball_data.setSizes[OtherAxes] = 1;
}
...
{
    HVect v = convert(point.x, point.y, nWinWidth, nWinHeight);

    Mouse(v);    // sets new mouse coordinates
    BeginDrag(); // indicates that dragging should begin
    Update();    // Alters the internal state of the arc-ball
}
...
void CArcBall::MouseUp(POINT point, int nWinWidth, int nWinHeight)
{
    ...
    Mouse(v);  // sets new mouse coordinates
    EndDrag(); // indicates that dragging should end
    Update();  // Alters the internal state of the arc-ball
    Value();   // extracts the current matrix transform
}
...
void CArcBall::MouseMove(POINT point, int nWinWidth, int nWinHeight)
{
    ...
    Update(); // Alters the internal state of the arc-ball
    Value();  // reads the matrix from the arc-ball
}
...
void CArcBall::DumpArcBall(FILE* file)
{
...
bool CArcBall::LoadArcBall(string fileName)
{
    ...
    if (fin.fail() == true)
    {
        cout << "Failed to load arc-ball parameters from config file " << fileName << "!\n";
        return false;
    }
    string param;
    while (fin >> param)
    {
        if (param == string("ARCBALL_Q_DOWN"))
        {
            fin >> ball_data.qDown.x >> ball_data.qDown.y >> ball_data.qDown.z >> ball_data.qDown.w;
        }
    }
    BeginDrag();
    Update();
